# File Locker Documentation

This directory contains comprehensive documentation for the File Locker project.

## 📚 Documentation Structure

### Core Documentation
- [ARCHITECTURE.md](ARCHITECTURE.md) - System architecture and design decisions
- [CONTRIBUTING.md](CONTRIBUTING.md) - Contribution guidelines for developers

### Deployment Guides
Located in [`deployment/`](deployment/):
- [PRODUCTION_DISTRIBUTION.md](deployment/PRODUCTION_DISTRIBUTION.md) - Complete production deployment guide
- [PRODUCTION_QUICK_START.md](deployment/PRODUCTION_QUICK_START.md) - Quick reference cheat sheet
- [RASPBERRY_PI_DEPLOYMENT.md](deployment/RASPBERRY_PI_DEPLOYMENT.md) - Raspberry Pi specific deployment
- [DISTRIBUTION_README.md](deployment/DISTRIBUTION_README.md) - Distribution files overview
- [QUICK_DEPLOY.md](deployment/QUICK_DEPLOY.md) - Quick deployment instructions

### Implementation History
Located in [`history/`](history/):
- [IMPLEMENTATION_COMPLETE.md](history/IMPLEMENTATION_COMPLETE.md) - Production distribution implementation summary
- [ADMIN_DASHBOARD_COMPLETE.md](history/ADMIN_DASHBOARD_COMPLETE.md) - Admin dashboard implementation
- [CLI_IMPLEMENTATION_SUMMARY.md](history/CLI_IMPLEMENTATION_SUMMARY.md) - CLI tool implementation details
- [frontend-implementation.md](history/frontend-implementation.md) - Frontend implementation summary
- [Context.md](history/Context.md) - Complete technical summary and project context
- [Design.md](history/Design.md) - Design decisions and patterns
- [IMPLEMENTATION.md](history/IMPLEMENTATION.md) - General implementation notes

## 🚀 Quick Links

### For Developers
- **Getting Started**: See [../README.md](../README.md) in the project root
- **Development Guide**: See [../DEVELOPER_GUIDE.md](../DEVELOPER_GUIDE.md)
- **Backend README**: [../backend/README.md](../backend/README.md)
- **Frontend README**: [../frontend/README.md](../frontend/README.md)
- **Frontend Quick Start**: [../frontend/QUICKSTART.md](../frontend/QUICKSTART.md)

### For Operators
- **Deployment**: Start with [deployment/PRODUCTION_QUICK_START.md](deployment/PRODUCTION_QUICK_START.md)
- **API Documentation**: [../backend/docs/openapi.yaml](../backend/docs/openapi.yaml)

### For Contributors
- **Contributing Guidelines**: [CONTRIBUTING.md](CONTRIBUTING.md)
- **Architecture Overview**: [ARCHITECTURE.md](ARCHITECTURE.md)

## 📖 Component Documentation

### Backend
- Location: `../backend/`
- Documentation: [../backend/README.md](../backend/README.md)
- API Specification: [../backend/docs/openapi.yaml](../backend/docs/openapi.yaml)

### Frontend
- Location: `../frontend/`
- Documentation: [../frontend/README.md](../frontend/README.md)
- Quick Start: [../frontend/QUICKSTART.md](../frontend/QUICKSTART.md)

## 🔍 Finding What You Need

- **"How do I set up my development environment?"** → [../DEVELOPER_GUIDE.md](../DEVELOPER_GUIDE.md)
- **"How do I deploy to production?"** → [deployment/PRODUCTION_QUICK_START.md](deployment/PRODUCTION_QUICK_START.md)
- **"How does the system work?"** → [ARCHITECTURE.md](ARCHITECTURE.md)
- **"What API endpoints are available?"** → [../backend/docs/openapi.yaml](../backend/docs/openapi.yaml)
- **"How was this feature implemented?"** → Check `history/` folder
- **"How do I contribute?"** → [CONTRIBUTING.md](CONTRIBUTING.md)
